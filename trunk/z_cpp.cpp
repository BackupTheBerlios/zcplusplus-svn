...
int recognize_bool_opt(const char* const x)
{
...
/*! 
 * Asks the preprocessor class whether it recognizes a given string as a language.
 * If so, sets stringopt::lang.  Otherwise, be ACID and don't update anything.
 *
...
{
	const char* test = CPreprocessor::echo_valid_lang(x);
